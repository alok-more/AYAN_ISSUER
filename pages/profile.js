...
export default function Profile() {
  ...
  const handlePostCredential = async (credential) => {
    ...
    const requestBody = {
      ...
      options: {
        proofType: "Ed25519Signature2019",
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {credentialData.map((credential, index) => {
              ...
              return (
                ...
                >
                  {/* Card Content */}
                  <div class="absolute top-2 left-0 w-full h-14 bg-black bg-opacity-20 text-white-800"></div>
                  ...
                  <div class="absolute bottom-0 right-0 w-24 h-24 bg-white bg-opacity-10 rounded-full -mr-12 -mb-12"></div>
                </div>
              );
            })}
          </div>
